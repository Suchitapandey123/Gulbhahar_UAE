...
export function ReviewEditForm({ 
  ...
  return (
    <div className={`space-y-${isMobile ? '3' : '4'}`}>
      {/* Edit Rating */}
      <div>
        <p className={`text-${isMobile ? 'xs' : 'sm'} text-gray-600 mb-${isMobile ? '1' : '2'}`}>
          Rating:
        </p>
      ...
      <div className="flex gap-2">
        ...
        >
          {updating ? (
            <>
              <div className={`w-${isMobile ? '3' : '4'} h-${isMobile ? '3' : '4'} border${isMobile ? '' : '-2'} border-white border-t-transparent rounded-full animate-spin`} />
