...
export function ReplySection({ reviewIndex, variant = "desktop" }) {
    ...
    <div className={`mt-${isMobile ? '3' : '4'}`}>
      ...
      <div className={`p-${isMobile ? '3' : '4'} bg-gray-50 rounded-lg border`}>
        <div className={`flex gap-${isMobile ? '2' : '3'}`}>
          <div className={`w-${isMobile ? '6' : '8'} h-${isMobile ? '6' : '8'} bg-red-900 rounded-full flex items-center justify-center flex-shrink-0`}>
            <User className={`w-${isMobile ? '3' : '4'} h-${isMobile ? '3' : '4'} text-white`} />
          </div>
          <div className="flex-1">
            <textarea
              ...
              rows="3"
            />
            <div className={`flex justify-end gap-${isMobile ? '2' : '3'} mt-${isMobile ? '2' : '3'}`}>
              <button
              ...
              >
                <Send className={`w-${isMobile ? '3' : '4'} h-${isMobile ? '3' : '4'}`} />
      ...
      {/* Display Replies */}
      {replies.length > 0 && (
        <div className={`mt-${isMobile ? '3' : '4'} space-y-${isMobile ? '2' : '3'}`}>
          {replies.map((reply, replyIdx) => (
            ...
            >
              <div className={`flex items-center gap-${isMobile ? '2' : '3'} mb-${isMobile ? '1' : '2'}`}>
                <div className={`w-${isMobile ? '5' : '7'} h-${isMobile ? '5' : '7'} bg-blue-600 rounded-full flex items-center justify-center`}>
                  <User className={`w-${isMobile ? '3' : '4'} h-${isMobile ? '3' : '4'} text-white`} />
                </div>
                <span className={`text-${isMobile ? 'xs' : 'sm'} font-medium text-gray-900`}>
                  {reply.user}
                </span>
                <span className={`text-${isMobile ? 'xs' : 'sm'} text-gray-500`}>
                  {reply.date}
                </span>
              </div>
              <p className={`text-${isMobile ? 'xs' : 'sm'} text-gray-700 ml-${isMobile ? '7' : '10'}`}>
                {reply.text}
              </p>
            </div>
          ))}
