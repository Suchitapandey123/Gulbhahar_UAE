...
const ContentSection = ({ page: initialPage }) => {
  ...
  useEffect(() => {
    if (initialPage && initialPage.additionalDetails) {
      const parsedPage = {
        ...initialPage,
        parsedAdditionalDetails: initialPage.additionalDetails.map(detail => 
          parseDetailsContent(detail)
        )
      };
        ...
        <div className="space-y-8 sm:space-y-12 lg:space-y-16">
          ...
          {page.parsedAdditionalDetails?.[1] && (
            <section>
              ...
              {page.parsedAdditionalDetails[1].highlight && (
          ...
          {finalRemaining && (
            <p className="text-gray-700 leading-relaxed text-sm sm:text-base lg:text-lg">
              ...
              {page.parsedAdditionalDetails[3].keyValues && page.parsedAdditionalDetails[3].keyValues.length > 0 && (
                <p className="text-gray-700 leading-relaxed text-sm sm:text-base lg:text-lg">
                  {page.parsedAdditionalDetails[3].keyValues.map(kv => kv.value).join(' ')}
                </p>
              )}
            </section>
          )}
...
>
  ...
  <p className="text-gray-600 text-sm sm:text-base">
    {kv.value}
              ...
              {(!page.parsedAdditionalDetails[5].keyValues || page.parsedAdditionalDetails[5].keyValues.length === 0) && 
               page.parsedAdditionalDetails[5].points && page.parsedAdditionalDetails[5].points.length > 0 && (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                  {page.parsedAdditionalDetails[5].points.map((point, index) => (
                    <div
                      key={index}
                      className="bg-white border border-red-200 rounded-lg sm:rounded-xl p-4 sm:p-6 shadow-sm hover:shadow-md transition-shadow duration-300"
                    >
