...
const StorySection = () => {
  ...
  return (
    ...
    >
      <div className="max-w-[1600px] mx-auto px-2 xs:px-4 sm:px-6 lg:px-8 relative z-10">
...
const ArtisanSection = () => {
  ...
  const artisans = [
    ...
    },
    {
      ...
      quote: "The intricate process of working with fabric and leather in an artistic craft that has tooling, cutting, moulding, and stitching them to create bespoke pieces of lasting quality.",
    ...
    <motion.div className="py-8 sm:py-16 lg:py-20 px-2 xs:px-4 sm:px-6 lg:px-8 bg-transparent">
      <div className="max-w-[1600px] mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-12 lg:gap-16">
          ...
          <motion.div
            className="lg:col-span-6 grid grid-cols-2 gap-4 sm:gap-5 lg:gap-6 order-2 lg:order-1"
            initial={{ opacity: 0, x: -100 }}
            whileInView={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.8 }}
            viewport={{ once: true }}
          >
            {collections.slice(0, 4).map((collection, index) => (
              <motion.div
                key={collection.name}
                className={`p-4 sm:p-5 rounded-xl cursor-pointer transition-all duration-300 border flex flex-col h-full aspect-auto overflow-hidden ${activeCollection === index
                ...
                whileTap={{ scale: 0.98 }}
              >
                <h3 className="text-md sm:text-lg lg:text-1xl line-clamp-2 font-bold mb-2 sm:mb-4">{collection.name}</h3>
                <p className={`mb-2 sm:mb-4 text-sm line-clamp-3 sm:text-sm ${activeCollection === index ? 'text-gray-200' : 'text-gray-600'}`}>
                  {collection.description}
                </p>
                 <div className=" mt-auto pt-3 sm:pt-2 lg:pt-3">
                <div className="text-sm sm:text-md lg:text-lg font-semibold mb-2 sm:mb-4">{collection.price}</div>
                <div className="flex flex-wrap gap-1 sm:gap-2">
                  {collection.features.map((feature, i) => (
                    <span
                      key={i}
                      className={`px-2 sm:px-2 rounded-full text-xs sm:text-xs border ${activeCollection === index
