...
export function RatingBreakdown({
  ...
  return (
    <div
      className={`space-y-${isMobile ? "2" : "3"} my-${isMobile ? "6" : "8"}`}
    >
      {breakdown.map(({ stars, count }) => (
        ...
        >
          ...
          <div
            className={`flex-1 h-${isMobile ? "2" : "3"} bg-gray-200 rounded-full overflow-hidden`}
          >
            <div
          ...
          <span
            className={`w-${isMobile ? "6" : "8"} text-${isMobile ? "xs" : "sm"} text-gray-600 font-medium`}
          >
            {count}
          </span>
        </div>
