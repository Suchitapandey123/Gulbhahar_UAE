...
interface ContentSectionProps {
  page: ParentCategoryPageData;
}

const ParentCategoryContentSection = ({ page }: ContentSectionProps) => {
          ...
          <section>
            ...
            {p.parsedAdditionalDetails[3].keyValues &&
              p.parsedAdditionalDetails[3].keyValues.length > 0 && (
                <p className="text-gray-700 leading-relaxed text-sm sm:text-base lg:text-lg">
                  {p.parsedAdditionalDetails[3].keyValues
                    .map((kv: { key: string; value: string }) => kv.value)
                    .join(" ")}
                </p>
              )}
          </section>
            ...
            {p.parsedAdditionalDetails[5].keyValues &&
              p.parsedAdditionalDetails[5].keyValues.length > 0 && (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                  {p.parsedAdditionalDetails[5].keyValues.map((kv: { key: string; value: string }, index: number) => (
                    <div
                      key={index}
     ...
     flex flex-col items-center justify-center text-center"
            ...
            {(!p.parsedAdditionalDetails[5].keyValues ||
              ...
              p.parsedAdditionalDetails[5].points.length > 0 && (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                  {p.parsedAdditionalDetails[5].points.map((point: string, index: number) => (
                    <div
                      key={index}
                      className="bg-white border border-red-200 rounded-lg sm:rounded-xl p-4 sm:p-6 shadow-sm hover:shadow-md transition-shadow duration-300"
                    >
        ...
        {p.parsedAdditionalDetails &&
          ...
          p.parsedAdditionalDetails.slice(8).map((detail: ParsedDetail, index: number) => (
            <section
              key={index + 8}
...
export default ParentCategoryContentSection;
